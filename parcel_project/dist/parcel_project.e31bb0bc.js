...
//그냥 createStore 하면 선그어지네 
//react를 사용하지 않기 때문에 DOM을 직접 수정해줘야 한다. 
//다음과 같이 js 상단에 수정할 DOM 노르를 가리키는 값을 미리 선언해 준다. 
var divToggle = document.querySelector('.toggle'); //html의 id 값이 toggle인 것을 불러와서 정의 하였다. 

var counter = document.querySelector('h1');
var btnIncrease = document.querySelector('#increase');
var btnDecrease = document.querySelector('#decrease'); //액션 이름 정의하기 

var TOGGLE_SWITCH = 'TOGGLE_SWITCH';
var INCREASE = 'INCREASE';
var DECREASE = 'DECREASE'; //액션 객체를 만드는 액션 생성 함수 작성하기
  ...
  switch (action.type) {
    case TOGGLE_SWITCH:
      return _objectSpread(_objectSpread({}, state), {}, {
        //불변성 유지를 해 주어야 한다. 
        toggle: !state.toggle //toggle 만 true면 false로 false면 true로 바꿔준다. 

      });

    case INCREASE:
      return _objectSpread(_objectSpread({}, state), {}, {
        counter: state.counter + action.difference //difference로 나타냈지 그냥 +1 하면 안되나
...
var store = (0, _redux.legacy_createStore)(reducer); //상태가 업데이트될 때마다 호출한다. 

var render = function render() {
  var state = store.getState(); //현재 상태를 불러온다. 이런게 어렵다. 
  //토글 처리
...
render();
store.subscribe(render); //액션을 발생시킨다. = dispatch
//파라미터에는 액션 객체를 넣어준다. 
...
divIncrease.onclick = function () {
  store.dispatch(increase());
};

divDecrease.onClick = function () {
...
//# sourceMappingURL=/parcel_project.e31bb0bc.js.map